...
if redis.call('SET', KEYS[1], ARGV[1]) then
    if redis.call('ZADD', KEYS[2], ARGV[2], KEYS[1]) then
        if redis.call('PUBLISH', KEYS[3], ARGV[1]) then
            return true
        else
...
"""
    ...
    def publish(self, _event):
        ...
        :return: Success.
        """
        event_key = 'EVENT:{}_{}:{}'.format(_event.topic, _event.action, _event.id)
        events_key = 'EVENTS:{}'.format(_event.topic)
        channel_key = 'EVENT:{}_{}'.format(_event.topic, _event.action)
        payload = json.dumps(_event.entity)
        timestamp = _event.ts

        return self.script_publish(keys=[event_key, events_key, channel_key], args=[payload, timestamp])
